...
add_to_log($course->id, 'simplelesson', 'view all', 'index.php?id='.$course->id, '');
...
if (! $simplelessons = get_all_instances_in_course('simplelesson', $course)) {
    notice(get_string('nosimplelessons', 'simplelesson'), new moodle_url('/course/view.php', array('id' => $course->id)));
}
...
foreach ($simplelessons as $simplelesson) {
    if (!$simplelesson->visible) {
        $link = html_writer::link(
            new moodle_url('/mod/simplelesson/view.php', array('id' => $simplelesson->coursemodule)),
            format_string($simplelesson->name, true),
            array('class' => 'dimmed'));
    } else {
        $link = html_writer::link(
            new moodle_url('/mod/simplelesson/view.php', array('id' => $simplelesson->coursemodule)),
    ...
    if ($course->format == 'weeks' or $course->format == 'topics') {
        $table->data[] = array($simplelesson->section, $link);
    } else {
        $table->data[] = array($link);
    }
}

echo $OUTPUT->heading(get_string('modulenameplural', 'simplelesson'), 2);
