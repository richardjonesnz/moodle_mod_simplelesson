...
if ($id) {
    $cm         = get_coursemodule_from_id(MOD_SIMPLELESSON_MODNAME, $id, 0, false, MUST_EXIST);
    $course     = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
    $moduleinstance  = $DB->get_record(MOD_SIMPLELESSON_TABLE, array('id' => $cm->instance), '*', MUST_EXIST);
} elseif ($n) {
    $moduleinstance  = $DB->get_record(MOD_SIMPLELESSON_TABLE, array('id' => $n), '*', MUST_EXIST);
    $course     = $DB->get_record('course', array('id' => $moduleinstance->course), '*', MUST_EXIST);
    $cm         = get_coursemodule_from_instance(MOD_SIMPLELESSON_TABLE, $moduleinstance->id, $course->id, false, MUST_EXIST);
} else {
...
if($showreport=='menu'){
	$PAGE->set_url(MOD_SIMPLELESSON_URL . '/reports.php', 
	array('id' => $cm->id)
	);
}else{
	$current_url = new moodle_url(MOD_SIMPLELESSON_URL . '/reports.php', 
	array('id' => $cm->id,'format'=>$format,'report'=>$showreport,'userid'=>$userid,'attemptid'=>$attemptid));
	
	$PAGE->set_url($current_url);
	$PAGE->navbar->add(get_string('reports'), new moodle_url(MOD_SIMPLELESSON_URL . '/reports.php', array('id' => $cm->id)));
...
if($CFG->version<2014051200){
	add_to_log($course->id, MOD_SIMPLELESSON_MODNAME, 'reports', "reports.php?id={$cm->id}", $moduleinstance->name, $cm->id);
}else{
	...
	$event->add_record_snapshot(MOD_SIMPLELESSON_MODNAME, $moduleinstance);
	$event->trigger();
} 
...
$renderer = $PAGE->get_renderer(MOD_SIMPLELESSON_FRANKY);
$reportrenderer = $PAGE->get_renderer(MOD_SIMPLELESSON_FRANKY,'report');
...
switch ($showreport){
	...
	case 'menu':
		echo $renderer->header($moduleinstance, $cm, $mode, null, get_string('reports', MOD_SIMPLELESSON_LANG));
	...
	case 'basic':
		$report = new mod_simplelesson_basic_report();
