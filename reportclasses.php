 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class mod_simplelesson_base_report {
	...
	public function fetch_head(){
		...
		foreach($this->fields as $field){
			$head[]=get_string($field,MOD_SIMPLELESSON_LANG);
		}
		return $head;
	}
...
*
*/
class mod_simplelesson_basic_report extends  mod_simplelesson_base_report {
	...
	public function fetch_formatted_heading(){
		...
		//$ec = $this->fetch_cache(MOD_SIMPLELESSON_TABLE,$record->englishcentralid);
		return get_string('basicheading',MOD_SIMPLELESSON_LANG);
	...
	public function process_raw_data($formdata,$paging=false){
		...
		$emptydata = array();
		$alldata = $DB->get_records(MOD_SIMPLELESSON_TABLE,array());
		if($alldata){
